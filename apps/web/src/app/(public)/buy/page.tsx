    ...
    async function fetchSales() {
      try {
        const { data, error } = await supabase
          ...
          .eq('status', 'available'); // available for purchase
        ...
        const formatted = (data || []).map((v: any) => ({
          ...
          sellingCurrency: v.selling_currency,
          status: v.status,
        ...
        console.error('Error fetching sales catalog:', err);
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="bg-[#F8F9FA] min-h-screen py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        <div className="space-y-2">
          <h1 className="text-3xl font-extrabold text-[#2C3E50] tracking-tight">Buy a Vehicle</h1>
          <p className="text-[#BDC3C7]">Explore premium vehicles for sale, complete with flexible installment and cash payment options.</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {filteredVehicles.map((vehicle) => (
              <VehicleCard key={vehicle.id} vehicle={{ ...vehicle, rentalRateDaily: null } as any} />
            ))}
          </div>
        ) : (
